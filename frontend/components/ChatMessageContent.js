...
function markdownToHtml(text) {
  ...
  const processInlineFormatting = text => {
    return (
      text
        ...
        .replace(
          /\[\[([^\]]+)\]\]/g,
          '<kbd class="px-1.5 py-0.5 text-xs font-semibold bg-gray-700 text-white rounded border border-gray-600 shadow-sm">$1</kbd>'
        )
    );
  };
  ...
  const formatSpecialMarkers = line => {
    return (
      line
        ...
        .replace(
          /^🗣️ (.+)/,
          '<hr class="my-3 border-t border-gray-300"/><p class="font-bold text-lg my-2 text-gray-200"><span class="mr-2">🗣️</span>$1</p>'
        )
        .replace(
          /^💡 (.+)/,
          '<hr class="my-3 border-t border-gray-300"/><p class="my-2 text-gray-200"><span class="font-bold text-yellow-300">💡 Tip: </span>$1</p>'
        )
        .replace(
          /^🔍 (.+)/,
          '<hr class="my-3 border-t border-gray-300"/><p class="my-2 text-gray-200"><span class="font-bold text-blue-300">🔍 Analysis: </span>$1</p>'
        )
        .replace(
          /^💬 (.+)/,
          '<hr class="my-3 border-t border-gray-300"/><p class="my-2 text-gray-200"><span class="font-bold text-green-300">💬 Note: </span>$1</p>'
        )
        .replace(
          /^⚠️ (.+)/,
          '<hr class="my-3 border-t border-gray-300"/><p class="my-2 text-gray-200"><span class="font-bold text-red-300">⚠️ Warning: </span>$1</p>'
        )
        .replace(
          /^🔑 (.+)/,
          '<hr class="my-3 border-t border-gray-300"/><p class="my-2 text-gray-200"><span class="font-bold text-purple-300">🔑 Key Point: </span>$1</p>'
        )
        // New section markers
        .replace(
          /^📋 (.+)/,
          '<hr class="my-3 border-t border-gray-300"/><p class="my-2 text-gray-200"><span class="font-bold text-blue-200">📋 Summary: </span>$1</p>'
        )
        .replace(
          /^🔧 (.+)/,
          '<hr class="my-3 border-t border-gray-300"/><p class="my-2 text-gray-200"><span class="font-bold text-orange-300">🔧 How-to: </span>$1</p>'
        )
        .replace(
          /^⭐ (.+)/,
          '<hr class="my-3 border-t border-gray-300"/><p class="my-2 text-gray-200"><span class="font-bold text-yellow-200">⭐ Highlight: </span>$1</p>'
        )
        .replace(
          /^❓ (.+)/,
          '<hr class="my-3 border-t border-gray-300"/><p class="my-2 text-gray-200"><span class="font-bold text-red-200">❓ Question: </span>$1</p>'
        )
        // Dividers
        ...
        .replace(
          /^\[x\] (.+)/,
          '<div class="flex items-start mt-2"><div class="bg-green-500 text-white rounded-sm w-5 h-5 flex items-center justify-center mr-2 mt-0.5 shadow-sm">✓</div><div class="flex-1">$1</div></div>'
        )
        .replace(
          /^\[ \] (.+)/,
          '<div class="flex items-start mt-2"><div class="border border-gray-400 rounded-sm w-5 h-5 mr-2 mt-0.5 shadow-inner"></div><div class="flex-1">$1</div></div>'
        )
        // Partial checkboxes
        .replace(
          /^\[~\] (.+)/,
          '<div class="flex items-start mt-2"><div class="bg-yellow-500 text-white rounded-sm w-5 h-5 flex items-center justify-center mr-2 mt-0.5 shadow-sm">⟨</div><div class="flex-1">$1</div></div>'
        )
    );
  };
  ...
  const processHeading = line => {
    ...
    if (h1Match)
      return `<h1 class="text-2xl font-bold mt-6 mb-4 pb-1 border-b border-gray-700">${processInlineFormatting(h1Match[1])}</h1>`;

    const h2Match = line.match(/^## (.+)/);
    if (h2Match)
      return `<h2 class="text-xl font-bold mt-5 mb-3 pb-1 border-b border-gray-800">${processInlineFormatting(h2Match[1])}</h2>`;

    const h3Match = line.match(/^### (.+)/);
    if (h3Match)
      return `<h3 class="text-lg font-bold mt-4 mb-2">${processInlineFormatting(h3Match[1])}</h3>`;

    const h4Match = line.match(/^#### (.+)/);
    if (h4Match)
      return `<h4 class="text-base font-bold mt-3 mb-2 text-gray-300">${processInlineFormatting(h4Match[1])}</h4>`;

    const h5Match = line.match(/^##### (.+)/);
    if (h5Match)
      return `<h5 class="text-sm font-bold mt-3 mb-1 text-gray-400">${processInlineFormatting(h5Match[1])}</h5>`;
        ...
        tableHeaders.forEach((header, i) => {
          const headerObj =
            typeof header === 'object'
              ? header
              : { content: header, align: 'text-left' };
        ...
        tableRows.forEach((row, rowIndex) => {
          ...
          row.forEach((cell, cellIndex) => {
            const align =
              ...
              typeof tableHeaders[cellIndex] === 'object'
                ? tableHeaders[cellIndex].align
                : 'text-left';
  ...
  const processLists = (line, indentLevel) => {
    ...
    const indentClass =
      nestLevel > 0 ? `ml-${Math.min(nestLevel * 2 + 5, 8)}` : 'ml-5';
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.trim().startsWith('>')) {
      ...
      if (!inQuoteBlock) {
        inQuoteBlock = true;
        html +=
          '<blockquote class="pl-4 border-l-4 border-blue-500 bg-gray-800 p-3 rounded-r-md my-4">';
      }
 ...
 */
const EnhancedChatMessageContent = ({
  text,
  sentiment,
  messageConfig = {},
}) => {
  ...
  const htmlContent = useMemo(
    () => markdownToHtml(text || 'No message content'),
    [text]
  );

  // Get sentiment display information
  const sentimentDisplay = useMemo(
    () => getSentimentDisplay(sentiment, text),
    [sentiment, text]
  );
    ...
    if (enableCopy) {
      ...
      codeBlocks.forEach(block => {
        ...
        if (!block.querySelector('.copy-button')) {
          ...
          button.className =
            'copy-button absolute top-2 right-2 bg-gray-700 hover:bg-gray-600 text-white rounded p-1 text-xs';
...
const HighlightedMessage = ({
  ...
  return (
    <div
      className={`relative my-4 p-4 rounded-md border-l-4 ${config.borderColor} ${config.bgColor}`}
    >
      {/* Title bar with icon */}
...
const CompleteChatMessage = ({
  ...
  return (
    <div
      className={`chat-message group p-4 mb-4 rounded-lg ${isAi ? 'bg-gray-800' : 'bg-gray-900'}`}
    >
      {/* Message header */}
      <div className='flex items-center mb-2'>
        {avatar && (
        ...
        <div className='flex flex-col'>
          <span
            className={`font-bold ${isAi ? 'text-blue-300' : 'text-green-300'}`}
          >
            {sender}
          </span>
