...
export default function LearningMaterials({ subject }) {
  ...
  const handleUpdate = async (materialId, updatedData) => {
    try {
      await api.put(`/api/materials/${materialId}`, updatedData);
    ...
    <div className='space-y-8'>
      <div className='bg-bg-dark-secondary rounded-xl p-6'>
        <div className='flex-1 grid grid-cols-1 md:grid-cols-3 gap-4'>
          <div>
            <label className='block text-text-medium mb-2 font-medium'>
              Search Content
            </label>
            <input
              type='text'
      ...
      <div className='space-y-6'>
        {materials.length > 0 ? (
          materials.map(material => (
            <div
              key={material._id}
              className='p-6 rounded-xl bg-gray-800 border-2 border-gray-600 hover:border-hf-blue transition-all duration-200'
            >
              ...
              {!material.content.startsWith('uploads/') && (
                <div className='mt-3'>
                  ...
                  >
                    <span
                      className={`transform transition-transform duration-200 ${expandedContents[material._id] ? 'rotate-90' : ''}`}
                    >
                      ▶
                    </span>
                    <span>Content</span>
              ...
              {material.tags?.length > 0 && (
                <div className='mt-2 flex flex-wrap gap-2'>
                  {material.tags.map(tag => (
                    <span
                      key={tag}
                      className='text-xs px-2 py-1 rounded bg-bg-dark-tertiary text-text-medium'
                    >
                      {tag}
                    </span>
                  ))}
