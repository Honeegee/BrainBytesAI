...
export default function Layout({ children, darkMode = false }) {
  ...
  return (
    <div className={`flex flex-col min-h-screen ${darkMode ? 'bg-gradient-to-t from-[#080c14] to-[#0f172a] text-text-light' : 'bg-white text-gray-900'}`}>
      ...
      <header className={`${darkMode ? 'bg-bg-dark' : 'bg-white'} sticky top-0 z-20 border-b border-border-dark`}>
        <nav className="w-full px-4 py-3 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center space-x-4">
            {isClient && !currentPath.includes('/login') && !currentPath.includes('/signup') && (
              <>
                <Link href="/dashboard" className={`text-sm ${darkMode ? 'text-text-light hover:text-hf-blue' : 'text-gray-900 hover:text-hf-blue'}`}>
                  Dashboard
                </Link>
                <Link href="/learning" className={`text-sm ${darkMode ? 'text-text-light hover:text-hf-blue' : 'text-gray-900 hover:text-hf-blue'}`}>
                  Learning
                </Link>
                <Link href="/chat" className={`text-sm ${darkMode ? 'text-text-light hover:text-hf-blue' : 'text-gray-900 hover:text-hf-blue'}`}>
                  Chat
                </Link>
        ...
        {isMobileMenuOpen && isClient && !currentPath.includes('/login') && !currentPath.includes('/signup') && (
          <div className={`md:hidden absolute top-full left-0 right-0 ${darkMode ? 'bg-bg-dark' : 'bg-white'} border-b border-border-dark shadow-lg py-2`}>
            <Link href="/dashboard" className={`block px-4 py-2 text-sm ${darkMode ? 'text-text-light hover:bg-bg-dark-secondary' : 'text-gray-900 hover:bg-gray-100'}`} onClick={() => setIsMobileMenuOpen(false)}>
              Dashboard
            </Link>
            <Link href="/learning" className={`block px-4 py-2 text-sm ${darkMode ? 'text-text-light hover:bg-bg-dark-secondary' : 'text-gray-900 hover:bg-gray-100'}`} onClick={() => setIsMobileMenuOpen(false)}>
              Learning
            </Link>
            <Link href="/chat" className={`block px-4 py-2 text-sm ${darkMode ? 'text-text-light hover:bg-bg-dark-secondary' : 'text-gray-900 hover:bg-gray-100'}`} onClick={() => setIsMobileMenuOpen(false)}>
              Chat
            </Link>
      ...
      {isClient && !currentPath.includes('/chat') && 
       ...
       !currentPath.includes('/signup') && (
        <div id="aiChat" className="fixed bottom-6 right-4 sm:right-6 z-50">
          <Link href="/chat">
            <button className="p-2 group scale-60 sm:scale-80 origin-bottom-right" aria-label="Toggle chat window">
