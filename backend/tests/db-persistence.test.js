...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/brainbytes';

// Helper to restart the MongoDB container
async function restartMongoContainer() {
  try {
    console.log('Finding MongoDB container...');
    const { stdout: containerId } = await execPromise('docker ps -q --filter "name=mongodb"');
    
    if (!containerId.trim()) {
      throw new Error('MongoDB container not found. Make sure it is running with a name containing "mongodb"');
    }
...
describe('Database Persistence Tests', () => {
  ...
  test('Data should persist across MongoDB container restarts', async () => {
    ...
    console.log('Successfully verified data persistence across container restart');
  });
  ...
  test('MongoDB should handle multiple records with persistence', async () => {
    ...
    const retrievedMessages = await Message.find({ chatId }).sort({ createdAt: 1 });
    ...
    for (let i = 0; i < messageCount; i++) {
      ...
      expect(retrievedMessages[i].isAiResponse).toEqual(messages[i].isAiResponse);
    }
