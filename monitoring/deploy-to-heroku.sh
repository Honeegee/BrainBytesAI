...
# Deploy Prometheus and Grafana to Heroku
# This script creates two separate Heroku apps for monitoring
...
# Set app names
PROMETHEUS_APP="brainbytes-prometheus-prod"
GRAFANA_APP="brainbytes-grafana-prod"
...
# Deploy Prometheus
git add .
git commit -m "Deploy Prometheus monitoring" || true
...
# Copy Grafana-specific files for deployment
cp grafana-heroku.yml heroku.yml
cp Procfile.grafana Procfile

# Deploy Grafana
git add .
git commit -m "Deploy Grafana monitoring" || true
