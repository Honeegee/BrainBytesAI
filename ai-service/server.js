...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    const detectQuestionType = input => {
      ...
      if (
        text.match(/^(what|who)\s+(is|are)\s+/i) ||
      ...
      if (
        text.match(/^(explain|how does|how do)\s+/i) ||
        text.match(/^(describe|tell me about)\s+/i)
      ) {
        return {
          type: 'explanation',
          text: text
            .replace(/^(explain|how does|how do)\s+/i, '')
      ...
      if (
        text.match(/^(give|show|provide)\s+(me\s+)?(an\s+)?example(s)?\s+/i) ||
        ...
        return {
          ...
          text: text
            .replace(
              /^(give|show|provide)\s+(me\s+)?(an\s+)?example(s)?\s+/i,
              ''
            )
